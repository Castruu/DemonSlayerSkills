...
public class Lua implements IActiveMethods{
        ...
        for (Entity entity : player.getNearbyEntities(10, 2, 10)) {
            ...
            if (entity instanceof LivingEntity) {
                Bukkit.getScheduler().runTaskAsynchronously(DSMain.getInstance(), () -> {
                    ...
                    for (double y1 = 0; y1 <= 2*Math.PI*2; y1 += 2*Math.PI*2 / 40F) {
                        double x1 = (radius * Math.cos(y1));
                        double z1 = (radius * Math.sin(y1));
                        location.add(x1, 1, z1);
                        radius -= 0.005;
                        player.getWorld().spigot().playEffect(location, Effect.WITCH_MAGIC, 0, 0, 0, 0, 0, 0, 5, 20);
                        location.subtract(x1, 1, z1);
                        try {
